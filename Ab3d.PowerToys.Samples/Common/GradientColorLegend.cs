...
namespace Ab3d.PowerToys.Samples.Common
{
    public class GradientColorLegend : Grid
    {
        ...
        public RenderTargetBitmap RenderToTexture(int size = 256, bool isHorizontal = true)
        {
            RenderTargetBitmap renderTargetBitmap;
            
            var gradientStartPoint = new Point(0, 0);
            Point gradientEndPoint;

            if (isHorizontal)
            {
                renderTargetBitmap = new RenderTargetBitmap(size, 1, 96, 96, PixelFormats.Pbgra32);
                gradientEndPoint = new Point(1, 0);
            }
            else
            {
                renderTargetBitmap = new RenderTargetBitmap(1, size, 96, 96, PixelFormats.Pbgra32);
                gradientEndPoint = new Point(0, 1);
            }


            // Create a new LinearGradientBrush with the same GradientStops but with start and end positions based on isHorizontal
            var linearGradientBrush = new LinearGradientBrush(GradientBrush.GradientStops, gradientStartPoint, gradientEndPoint);
            ...
            rectangle.Arrange(new Rect(0, 0, rectangle.DesiredSize.Width, rectangle.DesiredSize.Height));
